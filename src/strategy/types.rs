...
impl PortfolioData {
    ...
    pub fn log_portfolio_data(&self) {
        ...
        let mut market_data: Vec<(usize, String, Decimal, Decimal, Decimal, Decimal)> = self
            .market_addresses
            .iter()
            .enumerate()
            .map(|(i, &addr)| {
                ...
                (
                    ...
                    expected_return * Decimal::from_f64(10000.0).unwrap(),
                    std_dev * Decimal::from_f64(10000.0).unwrap(),
                    sharpe,
                    weight,
                )
            })
        ...
        market_data.sort_by(|a, b| {
            b.5.partial_cmp(&a.5)
                .unwrap_or(b.4.partial_cmp(&a.4).unwrap_or(std::cmp::Ordering::Equal))
        });
