...
pub struct MarketStateModel {
    ...
    pub pool_impact_amount: Option<Decimal>,
    pub pool_long_token_usd: Option<Decimal>,
    pub pool_short_token_usd: Option<Decimal>,
    pub pool_impact_token_usd: Option<Decimal>,
    pub open_interest_long: Option<Decimal>,
    pub open_interest_short: Option<Decimal>,
    pub open_interest_long_amount: Option<Decimal>,
    pub open_interest_short_amount: Option<Decimal>,
...
pub struct RawMarketStateModel {
...
impl RawMarketStateModel {
    pub fn from(market: &Market) -> Self {
        let timestamp: DateTime<Utc> = market.updated_at.unwrap().into();
        
        Self {
            market_address: to_checksum(&market.market_token, None),
            timestamp,
            ...
            pool_impact_amount: market.token_pool.map(|pool| pool.impact_token_amount),
            ...
            pool_impact_token_usd: market.token_pool.map(|pool| pool.impact_token_usd),
            open_interest_long: market.open_interest.map(|oi| oi.long),
            open_interest_short: market.open_interest.map(|oi| oi.short),
            open_interest_long_amount: market.open_interest.map(|oi| oi.long_amount),
            open_interest_short_amount: market.open_interest.map(|oi| oi.short_amount),
